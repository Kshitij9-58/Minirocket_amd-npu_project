...
# Modified for MiniRocket Project (bf16/f32 support)
...
# MAC Dimensions for different data types
microkernel_mac_dim_map = {
    "npu": {
        "bf16": (4, 8, 4),  # Brain Float 16 support
        "i8": (4, 8, 8),
        "i16": (4, 4, 4),
    },
    "npu2": {
...
def main():
    argparser = argparse.ArgumentParser(
        prog="MiniRocket Custom Kernel Generator",
        description="Generates a Matrix Multiplication kernel using bf16/f32 for high precision",
    )
    argparser.add_argument("--dev", type=str, choices=["npu", "npu2"], default="npu")
    
    # Defaults tailored for MiniRocket blocks (512x512)
    ...
    # CRITICAL CHANGE: Default to bf16 input and f32 output
    ...
    argparser.add_argument("--b-col-maj", type=int, choices=[0, 1], default=1) # Column major B usually helps
...
def my_matmul(
    ...
    # Note: We relax the strict integer check to allow bf16->f32
    # assert np.issubdtype(dtype_in, np.integer) == np.issubdtype(dtype_out, np.integer)

    # Ensure Output is larger or equal (f32 >= bf16)
    ...
    with mlir_mod_ctx() as ctx:
        ...
        @device(dev_ty)
        def device_body():
            # CORRECTION: Use square brackets [] for type hinting, NOT parentheses ()
            a_ty = np.ndarray[(m, k), np.dtype[dtype_in]]
            b_ty = np.ndarray[(k, n), np.dtype[dtype_in]]
            c_ty = np.ndarray[(m, n), np.dtype[dtype_out]]

            # AIE Core Function declarations
            # This generates: matmul_vectorized_bf16_f32
            ...
            # Stack size (Might need increase for floats, kept safe default)
            ...
            def sequence(A, B, C):
                # Runtime logic remains similar, handled by XRT
                rows_per_block = 4
